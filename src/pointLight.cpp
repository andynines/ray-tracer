...
Rgb PointLight::shade(Hit& hit, const Vec3& camPos) const {
	Rgb diffuse = hit.mat->calcDiffuse(hit.pos, hit.normal);
	Rgb result = hit.mat->ka * color.cwiseProduct(diffuse);
	...
	result += hit.mat->kd * localColor.cwiseProduct(diffuse) * std::max(hit.normal.dot(towardsLight), 0.0);
